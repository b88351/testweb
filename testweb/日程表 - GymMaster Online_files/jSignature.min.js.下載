...
function initialise_signature($elem, b64, image_dims) {
	// Making default parameters work before ES2015
	b64 = typeof b64 !== 'undefined' ? b64 : null;
	image_dims = typeof image_dims !== 'undefined' ? image_dims : null;

	sig_dict = {
		"signatureLine": false
	}
	if (image_dims !== null) {
		sig_dict['height'] = image_dims['height'];
		sig_dict['width'] = image_dims['width'];
	}

	$elem.jSignature(sig_dict);
	if (b64 !== null) $elem.jSignature('importData', b64);
